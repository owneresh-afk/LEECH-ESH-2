...
from bots.clients.telegram.helpers.user_utils import get_user_data, user_settings_text, update_user_ldata
...
async def get_user_settings(from_user: types.User, stype: str = "main"):
    ...
    if stype == "main":
        ...
        buttons.data_button(f"RClone Mode: {'ON' if rclone_mode else 'OFF'}", f"userset {user_id} toggle RCLONE_MODE")
        buttons.data_button(f"GDrive Mode: {'ON' if gdrive_mode else 'OFF'}", f"userset {user_id} toggle GDRIVE_MODE")
        ...
        buttons.data_button("Leech Split Size", f"userset {user_id} set LEECH_SPLIT_SIZE")
        buttons.data_button("Leech Destination", f"userset {user_id} set LEECH_DUMP_CHAT")
        ...
        buttons.data_button("Thumbnail Layout", f"userset {user_id} set THUMBNAIL_LAYOUT")
        
        as_doc = user_data.get("AS_DOCUMENT", False)
        buttons.data_button(f"Send As: {'Document' if as_doc else 'Media'}", f"userset {user_id} toggle AS_DOCUMENT")
        ...
        buttons.data_button("Buzzheavier Token", f"userset {user_id} set BUZZHEAVIER_TOKEN")
...
async def check_user_input(client: Client, user_id: int, chat_id: int, message: types.Message, setting_type: str, rfunc) -> None:
    ...
    async def filter_func(_, __, m: types.Message):
        return (m.from_user.id == user_id and 
                m.chat.id == chat_id and 
                (m.text or m.photo or m.document))
    ...
    try:
        ...
        if setting_type == "THUMBNAIL":
            if response_msg.photo:
                await update_user_ldata(user_id, setting_type, response_msg.photo.file_id)
        elif setting_type in ["RCLONE_CONFIG", "TOKEN_PICKLE"]:
            if response_msg.document:
                await update_user_ldata(user_id, setting_type, response_msg.document.file_id)
        else:
